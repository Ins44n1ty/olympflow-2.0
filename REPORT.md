...
Основной источник данных — `phys_book.pdf`, страницы `2..177`.
...
## 8. Topic-level temporal dynamics

Так как отдельные кластеры небольшие, был проведён дополнительный topic-level temporal analysis. Вместо анализа отдельных кластеров задачи агрегировались по автоматически интерпретированным темам из `tfidf_svd_kmeans`.

Категория `неоднозначная тема` была исключена из основной таблицы, потому что это техническая метка для автоматически неинтерпретированных кластеров, а не содержательная физическая тема.
...
- representative tasks closest to cluster centroid.
...
1. Weak labels не являются ручной semantic gold-разметкой. Они построены автоматически по локальным группам задач.
2. Sparse-методы могут выигрывать именно потому, что weak labels сильно согласованы с lexical similarity.
3. Dense embeddings могут находить смыслово похожие задачи из других weak-групп, но текущая оценка считает такие случаи ошибками.
4. Формулы и OCR-текст не нормализованы идеально.
5. Рисунки из условий пока не используются в основном пайплайне.
6. Topic labels получены автоматически, поэтому topic-level temporal analysis является exploratory analysis, а не финальной экспертной классификацией.
...
1. Сделать ручную semantic evaluation для небольшого подмножества задач.
2. Улучшить нормализацию формул и OCR-текста.
3. Проверить лемматизацию или стемминг для sparse baselines.
4. Попробовать cross-encoder reranking для top-k retrieval results.
5. Построить graph clustering на BM25 similarity graph.
6. Добавить vision-компонент для задач с рисунками.
7. Улучшить автоматическую интерпретацию кластеров через LLM-assisted labeling.
